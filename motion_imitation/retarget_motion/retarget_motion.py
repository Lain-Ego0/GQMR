"""Retarget dog motion-capture joint positions to a quadruped robot."""
...
mocap_motions = [
  ["pace", os.path.join(SCRIPT_DIR, "data/dog_walk00_joint_pos.txt"), 162, 201],
  ["trot", os.path.join(SCRIPT_DIR, "data/dog_walk03_joint_pos.txt"), 448, 481],
  ["trot2", os.path.join(SCRIPT_DIR, "data/dog_run04_joint_pos.txt"), 630, 663],
  ["canter", os.path.join(SCRIPT_DIR, "data/dog_run00_joint_pos.txt"), 430, 459],
  ["left_turn0", os.path.join(SCRIPT_DIR, "data/dog_walk09_joint_pos.txt"), 1085, 1124],
  ["right_turn0", os.path.join(SCRIPT_DIR, "data/dog_walk09_joint_pos.txt"), 2404, 2450],
]
...
def quaternion_rotate_point(point, quat):
  """Rotate a 3D point by a quaternion in [x, y, z, w] order."""
...
def calc_heading_rot(quat):
  """Return the quaternion containing only the rotation about the z axis."""
  forward = quaternion_rotate_point(FORWARD_DIR, quat)
...
def process_ref_joint_pos_data(joint_pos):
  ...
  for i in range(num_pos):
    curr_pos = proc_pos[i]
    curr_pos = quaternion_rotate_point(curr_pos, REF_COORD_ROT)
    curr_pos = quaternion_rotate_point(curr_pos, REF_ROOT_ROT)
...
def retarget_pose(robot, default_pose, ref_joint_pos):
  ...
  for i in range(len(REF_TOE_JOINT_IDS)):
    ...
    toe_offset_world = quaternion_rotate_point(toe_offset_local, heading_rot)
...
def main(argv):
  ...
  while True:
    ...
    for mocap_motion in mocap_motions:
      ...
      output_motion(retarget_frames,
                    os.path.join(SCRIPT_DIR, f"{mocap_motion[0]}.txt"))
